...
__global__ void matrixMul(char *a, char *b, char *c, int n) {
...
int main(int argc, char** argv) {
    ...
    int N = atoi(argv[1]);
    char *a, *b, *c; // Host matrices
    char *d_a, *d_b, *d_c; // Device matrices

    // Allocate memory for host matrices
    a = (char*)malloc(N * N * sizeof(char));
    b = (char*)malloc(N * N * sizeof(char));
    c = (char*)malloc(N * N * sizeof(char));
    ...
    cudaMalloc((void**)&d_b, N * N * sizeof(char));
    cudaMalloc((void**)&d_c, N * N * sizeof(char));

    // Copy host matrices to device
    cudaMemcpy(d_a, a, N * N * sizeof(char), cudaMemcpyHostToDevice);
    cudaMemcpy(d_b, b, N * N * sizeof(char), cudaMemcpyHostToDevice);
    ...
    cudaMemcpy(c, d_c, N * N * sizeof(char), cudaMemcpyDeviceToHost);
